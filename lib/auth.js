/* Carpeta encargada de tener toda la logica necesaria para las autentificaciones del usuario, contiene:
    ...
    - Habilitamos para que el usuario pueda cambiar su contraseña
...
// Logica para la creación de contraseña, recibe como parametro la nueva contraseña del usuario. Se encarga de actualizar la contraseña del usuario autenticado en Supabase. Si ocurre un error durante la actualización, se retorna ok:false junto con el mensaje correspondiente. Si la operación es exitosa, se retorna ok:true para continuar con el flujo normal.
export async function createUserPassword(password) {
    const supabase = getSupabaseBrowserClient();
    const { error } = await supabase.auth.updateUser({ password });
    if (error) {
        return { ok: false, error: "Error al guardar contraseña" };
    }
    return { ok: true };
}