...
// Crear un nuevo hilo con archivos y links externos (para cuentas o usuarios)
...
// Crear un nuevo hilo exclusivamente para usuarios
export async function crearHiloUsuario({
  titulo,
  contenido,
  idUsuarioCreador,
  idForoFuente,
  ...
  // 1. Crear el hilo (solo usuario, sin cuenta)
  const { data: hilo, error } = await supabase
    .from("Hilo")
    .insert([
      {
  ...
  if (error) {
    console.error("❌ Error creando hilo de usuario:", error);
    return { ok: false, mensaje: "Error al crear el hilo de usuario", error };
  }
  ...
  return { ok: true, mensaje: "Hilo de usuario creado exitosamente", hilo };
}
